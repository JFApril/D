...
const API_BASE = 'https://m.919tv9.com/api/web/live_lists';
const UA = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
const headers = {
  'User-Agent': UA,
  'Accept': 'application/json, text/plain, */*',
  'Referer': 'https://m.919tv9.com/'
};
const PAGE_SIZE = 20;
...
async function category(tid, pg, filter, extend) {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const stateIcon = getStatusIcon(item.state);
      const timeStr = formatTimestamp(item.match_time_int);
      const leagueName = safeString(item.league_name_zh);
      const remarks = [stateIcon, timeStr, leagueName].filter(Boolean).join(' ');
