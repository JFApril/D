...
def _hg_url_preview(url, limit=180):
    """只保留协议、主机和路径，避免把带签名 query/key 写入日志。"""
    try:
        ...
        if p.query:
            ...
            if len(base) + len(suffix) > limit:
                base = base[:max(0, limit - len(suffix))]
            return base + suffix
        return base[:limit]
    except Exception:
...
def _hg_parse_range(value):
    """解析单一 bytes Range；空结束值表示开放区间。"""
    try:
        text = str(value or "").strip()
        m = re.match(r"(?i)^bytes\s*=\s*(\d*)\s*-\s*(\d*)\s*(?:,|$)", text)
        if not m:
            return None, None
        lo = int(m.group(1)) if m.group(1) else 0
        hi = int(m.group(2)) if m.group(2) else -1
        if hi != -1 and hi < lo:
            return None, None
        return lo, hi
    except Exception:
...
def _hg_range_from_param(param):
    """从 localProxy 入参及其 header/headers 中读取 Range。"""
    ...
    for container_name in ("header", "headers"):
        try:
            container = param.get(container_name)
        except Exception:
            continue
        if isinstance(container, str):
            try:
                ...
                # 兼容 "Range: bytes=0-1\\nUser-Agent: ..." 形式的文本头
                try:
                    match = re.search(r"(?im)^\s*Range\s*:\s*(.+?)\s*$", container)
                    if match:
    ...
    try:
        for _ in range(5):
            ...
            try:
                ...
                _hg_diag(
                    "cdn_fetch_error hop=%d url=%s range=%s error=%s"
                    % (hop, _hg_url_preview(cur),
                       str((headers or {}).get("Range") or (headers or {}).get("range") or "-"),
                       type(exc).__name__)
                )
                return last, cur
            try:
                ...
                code = 0
            _hg_diag(
                "cdn_hop hop=%d status=%d url=%s range=%s"
                % (hop, code, _hg_url_preview(cur),
                   str((headers or {}).get("Range") or (headers or {}).get("range") or "-"))
            )
            if code in (301, 302, 303, 307, 308):
                ...
                try:
                    ...
                    pass
                if not loc:
                    _hg_diag("cdn_hop_missing_location hop=%d status=%d url=%s" % (hop, code, _hg_url_preview(cur)))
                    return None, cur
                # 相对Location拼全
                try:
                    if loc.startswith("/"):
        ...
        _hg_diag("cdn_fetch_exception hop=%d url=%s error=%s" % (hop, _hg_url_preview(cur), type(exc).__name__))
    return last, cur
...
class Spider(_BaseSpider):
    ...
    def localProxy(self, param):
        try:
            # FongMi/peekPRO 通常传解析后的 query dict；兼容少数壳直接传完整 proxy URL。
            input_kind = "dict" if isinstance(param, dict) else type(param).__name__
            if isinstance(param, str):
                try:
                    _pq = urlsplit(param)
                    param = dict(parse_qsl(_pq.query, keep_blank_values=True, max_num_fields=64))
                except Exception:
                    param = {}
            if not isinstance(param, dict):
                _hg_diag("entry_skip input=%s" % input_kind)
                return None
            typ = str(param.get("type") or "")
            if typ != "hgcenc":
                _hg_diag("entry_skip type=%s" % typ)
                return None
            src = unquote(str(param.get("url") or ""))
            kid = unquote(str(param.get("kid") or ""))
            spade = unquote(str(param.get("spade") or ""))
            range_header = _hg_range_from_param(param)
            try:
                range_header = str(range_header or "").strip()
            except Exception:
                range_header = ""
            _hg_diag(
                "entry input=%s src=%s range=%s kid_len=%d spade_len=%d"
                % (input_kind, _hg_url_preview(src), range_header or "-", len(kid), len(spade))
            )
            if not src.startswith("https://"):
                _hg_diag("reject bad_url src=%s" % _hg_url_preview(src))
                return [400, "text/plain", "bad url"]
            key_hex = _hg_spade_to_key(spade)
            if not key_hex:
                _hg_diag("reject key_missing spade_len=%d" % len(spade))
            ...
            req_lo, req_hi = _hg_parse_range(range_header)
            is_probe = False
            try:
                ...
                is_probe = False
            # TV 首播常见 bytes=0- 是开放区间；上游会忽略 Range 并返回整文件。
            # 取一段足够长的首包，避免播放器把短响应当 EOF 后回零重播。
            fetch_range_header = range_header
            range_was_open = False
            try:
                # bytes=0-、bytes=1048576- 等开放区间统一限制为 _HG_BOOTSTRAP_BYTES，
                # 让播放器按块续取，避免 CDN 忽略开放 Range 时反复下载整段文件。
                if req_lo is not None and req_hi == -1 and _HG_BOOTSTRAP_BYTES > 0:
                    _cap_end = req_lo + _HG_BOOTSTRAP_BYTES - 1
                    fetch_range_header = "bytes=%d-%d" % (req_lo, _cap_end)
                    range_was_open = True
            except Exception:
                pass
            if range_was_open:
                _hg_diag(
                    "range_cap original=%s fetch=%s bytes=%d"
                    % (range_header or "-", fetch_range_header, _HG_BOOTSTRAP_BYTES)
                )
            # 上游 CDN 用浏览器 UA 直拉；Referer 会触发 403（实测），绝不能加。
            headers = {"User-Agent": _USER_AGENT}
            if fetch_range_header:
                headers["Range"] = fetch_range_header
            try:
                plug = self._ensure()
            ...
            if r is None:
                _hg_diag(
                    "cdn_upstream_none range=%s fetch_range=%s final_url=%s fallback=302"
                    % (range_header or "-", fetch_range_header or "-", _hg_url_preview(_final_url))
                )
                return [302, "video/mp4", None, {"Location": src}]
            status = int(r.status_code or 0)
            _hg_diag(
                "cdn_response status=%d range=%s fetch_range=%s content_range=%s content_length=%s final_url=%s"
                % (
                    status,
                    range_header or "-",
                    fetch_range_header or "-",
                    _hg_header(r.headers, "Content-Range") or "-",
                    _hg_header(r.headers, "Content-Length") or "-",
                    _hg_url_preview(_final_url),
                )
            )
            if status not in (200, 206):
                try:
                    ...
                    pass
                _hg_diag("cdn_reject status=%d range=%s fallback=302" % (status, range_header or "-"))
                return [302, "video/mp4", None, {"Location": src}]
            ...
            try:
                ...
                pass
            _hg_diag(
                "cdn_total status=%d range=%s total=%d content_range=%s"
                % (
                    status,
                    range_header or "-",
                    total,
            ...
            if range_was_open:
                _hg_diag(
                    ...
                    % (
                        ...
                        len(data),
                        _HG_BOOTSTRAP_BYTES,
                        1 if len(data) >= _HG_BOOTSTRAP_BYTES else 0,
                    )
                )
            if not data:
                _hg_diag(
                    "cdn_empty status=%d range=%s fetch_range=%s final_url=%s fallback=302"
                    % (status, range_header or "-", fetch_range_header or "-", _hg_url_preview(_final_url))
                )
                return [302, "video/mp4", None, {"Location": src}]
            if range_was_open and req_lo is not None:
                lo = req_lo
                hi = req_lo + len(data) - 1
            elif hi < lo:
                hi = lo + len(data) - 1
            ...
            if is_probe:
                _hg_diag(
                    "decrypt skip reason=probe range=%s bytes=%d"
                    % (range_header or "-", len(buf))
                )
                try:
            ...
            try:
                ...
                mv = None
            _hg_diag(
                "moov status=%d range=%s total=%d bytes=%d has_moov=%d moov_end=%d"
                % (status, range_header or "-", total, len(buf), 1 if has_moov else 0, moov_end)
            )
            if not mv:
            ...
            try:
                if v_iv and vsizes and voffs:
                    ...
                    try:
                        v_done = _hg_decrypt_range(buf, key_hex, v_iv, vsizes, vo2, 0, len(buf))
                    except Exception as exc:
                        decrypt_error = type(exc).__name__
                if a_iv and asizes and aoffs:
                    ao2 = [c - lo for c in aoffs]
                    try:
                        a_done = _hg_decrypt_range(buf, key_hex, a_iv, asizes, ao2, 0, len(buf))
                    except Exception as exc:
                        decrypt_error = type(exc).__name__
            except Exception as exc:
                decrypt_error = type(exc).__name__
            _hg_diag(
                "decrypt status=%d range=%s v_iv=%d a_iv=%d v_samples=%d a_samples=%d v_done=%d a_done=%d error=%s"
                % (
                    status,
                    range_header or "-",
                    1 if v_iv else 0,
