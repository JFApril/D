# -*- coding: utf-8 -*-
# 源码
# VIP兔 V2.0 - 修复详情页不显示：默认 main_api 域名(tianwei.qzz.io)已死亡返回反诈HTML，
# 且网站配置无 hot_db 字段导致 main_api 无法被真实配置覆盖；详情改用活源按片名回查兜底。
...
class Spider(Spider):
    ...
    main_api = "https://cms.txnp.cn/api.php/provide/vod/"
    category_map = {
        "2": "剧集",
        "1": "电影",
        "3": "综艺",
    ...
    fallback_search_sources = [
        ...
        ("无广2", "https://cms.txnp.cn/api.php/provide/vod/"),
    ...
    def _load_public_config(self):
        ...
        for line in str(config.get("search_api") or "").splitlines():
            ...
            if "m3u8" in source_api.lower() or "/provide/vod" in source_api:
                sources.append((source_name, source_api))
        if sources:
            self.search_sources = self._order_media_sources(sources)[:4]
        if built_in_parsers:
    ...
    def _encode_id(self, source_name, source_api, vod_id, title=""):
        payload = json.dumps(
            {"name": source_name, "api": source_api, "id": str(vod_id), "t": str(title or "")},
            ensure_ascii=False,
            separators=(",", ":"),
        ).encode("utf-8")
    ...
    def _decode_id(self, value):
        ...
        try:
            ...
            if source_api.startswith(("https://", "http://")) and (vod_id or title):
                return source_name, source_api, vod_id, title
        except Exception:
            pass
        return "VIP兔", self.main_api, value, ""

    def _build_item(self, item, source_name, source_api, title_override=""):
        ...
        return {
            "vod_id": self._encode_id(source_name, source_api, vod_id, title),
    ...
    def _find_direct_detail(self, title, original=None):
        wanted = self._normalize_title(title)
        if not wanted:
            return None
        for source_name, source_api in self.search_sources:
            data = self._request_json(source_api, params={"ac": "list", "wd": quote(title), "pg": 1}) or {}
            for candidate in data.get("list") or []:
                cand_title = candidate.get("vod_name", candidate.get("title", ""))
                if self._normalize_title(cand_title) != wanted:
                    continue
                cand_id = candidate.get("vod_id", candidate.get("id", ""))
                if not cand_id:
                    continue
                detail = self._request_json(source_api, params={"ac": "detail", "ids": cand_id}) or {}
                matches = detail.get("list") or []
                if not matches:
                    continue
                item = matches[0]
                if original and not self._same_media_version(original, item):
                    continue
                return item
        return None
    ...
    def detailContent(self, ids):
        ...
        source_name, source_api, vod_id, title = self._decode_id(token)
        if not vod_id and not title:
            return {"list": []}

        item = None
        if vod_id and source_api:
            data = self._request_json(source_api, params={"ac": "detail", "ids": vod_id}) or {}
            movies = data.get("list") or []
            if movies:
                item = movies[0]

        if not item and title:
            item = self._find_direct_detail(title)

        if not item:
            vod = {
                "vod_id": str(token),
                "vod_name": self._safe_text(title) or "未知",
                "vod_pic": self.default_pic,
                "vod_remarks": "暂无播放源",
                "vod_content": "详情获取失败，可尝试搜索同名资源。",
                "vod_play_from": "",
                "vod_play_url": "",
            }
            return {"list": [vod]}
        ...
        vod = {
            "vod_id": str(token),
            "vod_name": self._safe_text(item.get("vod_name", title)),
            ...
            "vod_content": "剧情介绍:" + (item.get("vod_content", item.get("vod_blurb", "")) or ""),
    ...
    def _find_direct_play_sources(self, original):
        ...
        seen_names = set()
        for source_name, source_api in self.search_sources:
            data = self._request_json(source_api, params={"ac": "list", "wd": quote(original.get("vod_name", "")), "pg": 1}) or {}
    ...
    def searchContent(self, key, quick=False, pg=1):
        ...
        for source_name, source_api in self.search_sources:
            data = self._request_json(
                source_api,
                params={"ac": "list", "wd": quote(keyword), "pg": page},
            ) or {}
            try:
