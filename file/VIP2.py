...
class Spider(BaseSpider):
    API = "https://xk21128.xkgzs.xyz/api/vod/"
    AES_KEY = b"11620jkjkSdkeYun"
    PAGE_SIZE = 36
    ...
    def detailContent(self, ids):
        ...
        for source in data.get("vod_play_url_list", []):
            ...
            if source_id is None:
                continue
            name = player.get("player_name") or code or "播放"
            if "维护" in name:
                continue
            episodes = []
            has_direct = False
            ...
            if episodes:
                is_4k = "4k" in name.lower()
                all_sources.append((name, "#".join(episodes), has_direct, is_4k))
        all_sources.sort(key=lambda s: (not s[3], not s[2]))
        play_from = [s[0] for s in all_sources]
        play_urls = [s[1] for s in all_sources]
        item = self._vod(vod)
        item.update({
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        elif "youku.com" in lower:
            header["Referer"] = "https://v.youku.com/"
        elif "bilibili.com" in lower or "bilivideo.com" in lower:
            header["Referer"] = "https://www.bilibili.com/"
        elif "iqiyi.com" in lower:
            header["Referer"] = "https://m.iqiyi.com/"
        elif "mgtv.com" in lower:
            header["Referer"] = "https://www.mgtv.com/"
        elif "sohu.com" in lower:
            header["Referer"] = "https://tv.sohu.com/"
        return {
