...
# 改造说明：
#   1) 保留原有 HTML 列表/详情解析作为兜底（站点可达时正常工作）；
#   2) 引入看剧AI 风格的多线路解析框架：playback token + resolve 线路列表 +
#      provider/权重排序 + resolve_ticket 换真实地址 + m3u8/mp4 直链优先；
#   3) 站点/接口不可达时返回空列表并打日志，便于排查（当前两端均有 403 反爬，
#      需站点恢复或提供可达接口后方可正常出数据）。
import sys
import re
import json
import time
