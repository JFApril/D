...
let host = 'https://zzzb18vip.app';
let apiHost = 'https://ugcaamdqtywxlcnuucb01.k8v4dh4.app';
...
async function fetchLiveIndex() {
  ...
  const data = safeJson(r.content);
  const items = data.data ? 
    (data.data.ongoingLivestreams || []).concat(data.data.anchorLivestreams || []).concat(data.data.streamingAnchorRanking || []) : [];
  items.forEach(it => {
    if (it.houseId) houseCache[it.houseId] = it;
  });
  return data;
}
...
async function detail(id) {
  ...
  let it = houseCache[houseId] || {};
  
  if (!it.houseId) {
    await fetchLiveIndex();
    it = houseCache[houseId] || {};
  }
